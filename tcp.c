    ...
    if (TcpTable[i].myPort != 0) {
      if (TcpTable[i].status == TCP_ESTABLISHED) {
        printf("TCP:%d:%u=%s:%u-%s:%u:%s\n",
            i, TcpTable[i].myPort, inet_ntop(AF_INET, &Param.vip, buf1, sizeof(buf1)),
            TcpTable[i].myPort, inet_ntop(AF_INET, &TcpTable[i].dstAddr, buf2,
            sizeof(buf2)), TcpTable[i].dstPort, TcpStatusStr(TcpTable[i].status));
      } else {
        printf("TCP:%d:%u=%s:%u:%s\n", i,
            TcpTable[i].myPort, inet_ntop(AF_INET, &Param.vip, buf1, sizeof(buf1)),
            TcpTable[i].myPort, TcpStatusStr(TcpTable[i].status));
      }
    }
  }
...
int TcpSendFin(int soc, int no)
{
  ...
  tcp->check = TcpChecksum(&Param.vip, &TcpTable[no].dstAddr,
      IPPROTO_TCP, (u_int8_t *)sbuf, ptr - sbuf);

  printf("=== TCP ===[\n");
  IpSend(soc, &Param.vip, &TcpTable[no].dstAddr, IPPROTO_TCP, 1,
      Param.IpTTL, sbuf, ptr - sbuf);
  print_tcp(tcp);
  printf("]\n");
...
int TcpSendRstDirect(int soc, struct ether_header *r_eh, struct ip *r_ip, struct tcphdr *r_tcp)
{
  ...
  tcp->check = TcpChecksum(&Param.vip, &r_ip->ip_src, IPPROTO_TCP,
      (u_int8_t *)sbuf, ptr - sbuf);

  printf("=== TCP ===[\n");
  IpSend(soc, &Param.vip, &r_ip->ip_src, IPPROTO_TCP, 1, Param.IpTTL,
      sbuf, ptr - sbuf);
  print_tcp(tcp);
  printf("]\n");
...
int TcpSendData(int soc, u_int16_t sport, u_int8_t *data, int len)
{
  ...
  printf("=== TCP ===[\n");
  IpSend(soc, &Param.vip, &TcpTable[no].dstAddr, IPPROTO_TCP, 0,
      Param.IpTTL, sbuf, ptr - sbuf);
  print_tcp(tcp);
  print_hex(data, len);
  printf("]\n");
  ...
  if ( (no = TcpSearchTable(ntohs(tcp->dest))) != -1 ) {
    if (TcpTable[no].rcv.nxt != 0 && ntohl(tcp->seq) != TcpTable[no].rcv.nxt) {
      printf("TcpRecv:%d:seq(%u) != rcv.nxt(%u)\n", no,
          ntohl(tcp->seq), TcpTable[no].rcv.nxt);
    } else {
    ...
    printf("TcpRecv:%d:%s:S[%u,%u,%u,%u]:R[%u,%u,%u]\n", no,
        ...
        TcpTable[no].snd.wnd,
        TcpTable[no].snd.iss,
        TcpTable[no].rcv.nxt - TcpTable[no].rcv.irs,
        TcpTable[no].rcv.wnd, TcpTable[no].rcv.irs);
  } else {
